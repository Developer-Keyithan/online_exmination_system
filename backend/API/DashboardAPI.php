...
class DashboardAPI
{
    ...
    public function adminDashboard()
    {
        try {
            ...
            $stmt = $this->db->query("SELECT COUNT(*) FROM users WHERE user_group = 6 AND status = 1");
            $stats['students'] = $stmt->fetchColumn();
            
            $stmt = $this->db->query("SELECT COUNT(*) FROM users WHERE user_group = 5 AND status = 1");
            $stats['lecturers'] = $stmt->fetchColumn();
            
            // Get recent users
            $stmt = $this->db->prepare("
                SELECT id, name, email, user_group, created_at
                FROM users 
                WHERE status = 1
            ...
            // Get upcoming exams (you'll need to have exam_info table)
            $upcomingExams = [];
