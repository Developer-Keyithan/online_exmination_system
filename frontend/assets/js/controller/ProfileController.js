app.controller('ProfileController', ['$scope', '$http', '$timeout', '$filter', function ($scope, $http, $timeout, $filter) {
    ...
    $scope.passwordsMatch = function () {
        return $scope.passwordData.new_password === $scope.passwordData.confirm_password;
    };

    // Check if password form is valid
    $scope.isPasswordFormValid = function () {
        return $scope.passwordData.current_password &&
            $scope.passwordData.new_password &&
            $scope.passwordData.confirm_password &&
            $scope.passwordsMatch() &&
            $scope.passwordStrength >= 3; // Require at least 3/4 requirements
    };
    ...
    $scope.deleteAccount = function () {
        ...
        $http.delete(window.baseUrl + '/API/profile/delete-account')
            .then(function (response) {
                if (response.data.success) {
                    showNotification('Account deletion requested. You will be logged out shortly.', 'info');
                    // Redirect to logout
                    $timeout(function () {
